            ...
            <div class="grid md:grid-cols-3 gap-8">
                <div class="md:col-span-2 space-y-4">
                        ...
                        <div class="bg-[#1a1a1a] border border-gray-700 rounded-lg p-6 flex gap-6">
                            ...
                            <div class="flex flex-col items-end justify-between">
                                ...
                                <div class="flex flex-col items-end gap-2">
                                    <div class="flex items-center gap-3 bg-[#2a2a2a] p-1 rounded-lg">
                                        <form action="{{ route('cart.update', $id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="quantity" value="{{ $item['quantity'] - 1 }}">
                                            <button type="submit" class="w-8 h-8 flex items-center justify-center text-white hover:bg-[#333] transition rounded">-</button>
                                        </form>
                                    ...
                                    <p class="text-white font-bold">
                                        Sub: IDR {{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <form action="{{ route('cart.clear') }}" method="POST" class="mt-4" onsubmit="return confirm('Are you sure you want to clear your cart?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-400 hover:text-red-300 transition flex items-center gap-2">
                ...
                <div class="md:col-span-1">
                    <div class="bg-[#1a1a1a] border border-gray-700 rounded-lg p-6 sticky top-32">
                        ...
                        <a href="{{ route('checkout') }}" class="block w-full bg-white text-black text-center py-4 rounded-lg hover:bg-gray-200 transition font-bold uppercase tracking-wider mb-3">
                            Proceed to Checkout
                        </a>

                        <a href="{{ route('build') }}" class="block w-full bg-[#2a2a2a] text-white text-center py-3 rounded-lg hover:bg-[#333] transition border border-gray-700">
                            Continue Building
                        </a>
                    </div>
                </div>
